...
if st.session_state.page == "login" and not st.session_state.logged_in:
    ...
    if username and password:
        if st.button("Login"):
            ...
            match = df[(df['Username'] == username) & (df['GApassword'] == password)]
            if not match.empty:
    ...
    if fill:
        if st.button("Create Account"):
            ...
            if name in df["Username"].values:
                ...
                new_row = pd.DataFrame([[name, email, key_conf]], columns=["Username", "E-Mail", "GApassword"])
                conn.write(new_row, worksheet=SHEET_NAME, mode="a")
...
if st.session_state.logged_in:
    ...
    if df is not None:
        ...
        graph_type = st.sidebar.selectbox("Chart type", ["Auto Suggestion","Bar","Line","Scatter","Pie","Histogram","Box","Area","Heatmap"])
    ...
    if st.session_state.get('show_history', False):
        ...
        history_folder = os.path.join(base_folder, st.session_state.current_user)
